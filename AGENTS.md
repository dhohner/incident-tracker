...
- `bun run format`: run Prettier and rewrite files in-place.
- `bun run format:check`: run Prettier in dry-run mode (`--check`) for CI/pre-PR validation.
...
- Run `bun run format` after substantial edits to keep diffs clean.
- Use `bun run format:check` before opening PRs or in CI to catch formatting drift without changing files.
- Prettier skips generated/output paths listed in `.prettierignore` (`build`, `convex/_generated`, `node_modules`, `.react-router`).

## Code Clarity & Simplification Standards

- Preserve exact behavior when refactoring: do not change outputs, side effects, data flow, or edge-case handling.
- Prefer explicit and readable control flow over dense expressions; avoid nested ternaries for multi-branch logic.
- Keep logic close to where it is used, but extract helpers when the same behavior appears in multiple places.
- Remove unnecessary indirection, duplicated logic, and dead/obvious comments that do not add intent.
- Use names that describe intent (`allTickets`, `configuredTicketSeverity`) instead of generic names (`data`, `value`) when context is non-trivial.
- Avoid adding abstractions, patterns, or new components unless they are necessary for maintainability of the requested change.
- Favor small, composable functions with clear responsibilities; do not collapse unrelated concerns into one block.
