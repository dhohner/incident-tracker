...
export const TrackedTicket = Object.assign(
  ({ children }: TrackedTicketProps) => (
    <div className="flex flex-col gap-2 mb-4 rounded-[18px] border border-[rgba(54,214,194,0.4)] bg-[linear-gradient(135deg,rgba(54,214,194,0.15),transparent_55%),rgba(12,15,24,0.96)] p-[18px] shadow-[0_22px_40px_rgba(4,8,18,0.6)]">
      {children}
    </div>
  ),
  {
    ...
    Key: ({ children }: TrackedTicketProps) => (
      <span className="font-[var(--font-mono)] text-sm text-(--ink-500)">
        {children}
      </span>
    ),
